...
import {MatCheckbox, MatCheckboxChange, MatDialog, MatSelectChange, MatTableDataSource} from '@angular/material';
...
export class MainComponent implements OnInit, OnDestroy, AfterViewInit, AfterContentInit {
  ...
  disableStartStopButton: boolean = false;
  ...
  onChangeRoute() {
    this.activatedRoute.queryParams.subscribe((params) => {
      ...
      if (params.project_id && params.event === '1' || params.project_id && params['project_status']) {
        this.apiService.getSavedProjectById(params.project_id).subscribe((project) => {
          ...
          if (project.connection_settings.source.status !== 'UNTESTED') {
    ...
    this.activatedRoute.queryParams.subscribe((params) => {
      if (params.project_id && params.event === '2' || params.project_id) {
        this.apiService.getSavedProjectById(params.project_id).subscribe((project) => {
          this.projectForm = this.initializeForm(project);
          this.source_cluster_status = project.connection_settings.source.status;
          this.destination_cluster_status = project.connection_settings.destination.status;
  ...
  onGetIndices() {
    ...
    this.source_index_list.patchValue([]);
    // for (let i = 0; i < this.source_index_list.length; i++) {
    //   this.source_index_list.value[i].is_checked = false;
    // }
    // for (let i = 0; i < this.source_template_list.length; i++) {
    //   this.source_template_list.value[i].is_checked = false;
    // }


    this.showSpinner = true;
    this.apiService.getSources(this.projectForm.value).subscribe(project => {
      this.cdr.markForCheck();
  ...
  checkRowOfSourceIndexTable(event: MatCheckboxChange, row: ISourceIndexList) {
    ...
    const checkedRowIndex = this.source_index_list.value.indexOf(row);
    this.source_index_list.value.forEach(el => {
      if (this.source_index_list.value.indexOf(el) === checkedRowIndex) {
    ...
    this.apiService.prepareProject(project_id).subscribe((project) => {
      if (project) {
        ...
        if (project.status === 'ERROR' || project.status === 'WARNING') {
          this.disableStartStopButton = true;
        }
        if (project.status === 'PASS') {
          this.disableStartStopButton = false;
        }
      }
    }, error => {
      this.toastr.error(`Server not responding! Please try again later`, '', {
  ...
  onSelectAllAfterFilter(event: MatCheckboxChange) {
    ...
    if (event.checked) {
      ...
      this.selectionIndexList = new SelectionModel<ISourceIndexList>(false, []);
    }
  }
