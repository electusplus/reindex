...
public class ReindexTaskExecutor implements Runnable {
    ...
    public ReindexTaskExecutor(final String projectId, final ReindexTaskPOJO task, RestHighLevelClient client, final long taskRefreshInterval) {
    ...
    public void run() {
        ...
        try {
            ...
            } while (!reindexTaskStatus.isDone());
            Response lowResponse = lowLevelClient.performRequest(new Request("GET", "_tasks/" + taskId));
    ...
    private void stopIfFailure() {
        if (!isContinueONFailure) {
            logger.error("There is a failure in the reindex of the index: " + reindexTaskStatus.getSourceIndex() + " The reindex process will be terminated! ");
