...
import com.electusplus.reindex.data_warehouse.ReindexSettingsController;
...
//                try {
//                    transaction.setName("Get sources");
//                    transaction.setType(Transaction.TYPE_REQUEST);
//                    logger.info("Got request for get sources from Elasticsearch with body: " + request.body());
...
//                }
            ...
            post("/test_cluster/:id", (request, response) -> {
                ...
                if (responseBody.contains("error")) {
                    response.status(502);
//                    response.body("{\"cluster_status\" : \"ERROR\"}");
                }
                return responseBody;
            });
