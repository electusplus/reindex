package com.electusplus.reindex.reindex_execution_plan_builder.plan_validation;

import com.electusplus.reindex.constants.EReindexAlgorithms;
import com.electusplus.reindex.constants.EValidationTasksSettings;
import com.electusplus.reindex.project_settings.ProjectPOJO;
import com.electusplus.reindex.project_settings.ReindexSettingsPOJO;
import com.electusplus.reindex.reindex_execution_plan_builder.dao.alias.IsAliasExistsDAO;
import com.electusplus.reindex.reindex_execution_plan_builder.dao.cluster_health.IsClusterRespond;
import com.electusplus.reindex.reindex_execution_plan_builder.dao.cluster_settings.IsReindexRemoteClusterDefined;
import com.electusplus.reindex.reindex_execution_plan_builder.dao.index.IsDateFieldExistsAndHaveDateTypeDAO;
import com.electusplus.reindex.reindex_execution_plan_builder.dao.index.IsIndexExistsDAO;
import com.electusplus.reindex.reindex_execution_plan_builder.dao.ingest_pipeline.IsPipelineExistsDAO;
import com.electusplus.reindex.reindex_execution_plan_builder.reindex_algorithms.ReindexAlgorithmPOJO;
...
public class ValidationPlanBuilder {
            ...
            if (reindexPlan.getReindexJobs().size() == 0) {
                ...
                if (currentAlgorithm.size() > 0 && currentAlgorithm.get(0).getReindexAlgorithmName().equals(EReindexAlgorithms.TIME_ORIENTED.getVarForUI())) {
                    ...
                    reindexPlan.getReindexJobs().values().stream()
                            ...
                            .forEach(job -> generateValidationForTimeSeriasAlgorithmTask(job.getIndexName(), dateField));
    ...
    private void generateValidationForTimeSeriasAlgorithmTask(final String index, final String dateField) {
