...
import com.electusplus.reindex.elasticsearch.DataPeriodFromEs;
...
public class TimeSeriesReindexAlgorithm extends ReindexAlgorithmPOJO implements IReindexAlgorithm {
    ...
    public TimeSeriesReindexAlgorithm(ReindexAlgorithmPOJO reindexAlgorithm) {
    ...
    public ReindexTaskPOJO generateRequestFromReprocessing(final EsSettings source, final String index, final boolean isRemote,
    ...
    @Override
    public List<ReindexTaskPOJO> generateRequests(final EsSettings source, final String index, final boolean isRemote, final String projectId) throws ClusterConnectionException {
...
//        RestHighLevelClient client = elasticsearchDbProvider.getHighLevelClient(source, projectId);
//        ElasticsearchController elasticsearchController = new ElasticsearchController();
//        String dateField = algorithmParams.stream()
//                .filter(x -> x.getLabel().equals("Date field"))
//                .map(AlgorithmParam::getActualValue)
//                .collect(Collectors.toList())
//                .get(0).toString();
//
//        String stringTimeChunk = algorithmParams.stream()
//                .filter(x -> x.getLabel().equals("Time chunk"))
//                .map(AlgorithmParam::getActualValue)
//                .collect(Collectors.toList())
//                .get(0).toString();
//
//        String dateFormatFromParams = algorithmParams.stream()
//                .filter(x -> x.getLabel().equals("Date format"))
//                .map(AlgorithmParam::getActualValue)
//                .collect(Collectors.toList())
//                .get(0).toString();
//
//        long timeChunk = Integer.parseInt(stringTimeChunk) * 1000 * 60;
//
//        SimpleDateFormat dateFormat = new SimpleDateFormat(dateFormatFromParams);
//
//        DataPeriodFromEs starAnEndDate = elasticsearchController.getStartAndEndDateOfIndex(client, index, dateField);
...
//                        GeneralUtils.convertLongToDateString(fromDate + timeChunk, dateFormat));
//            }
//            ReindexTaskPOJO reindexTask = new ReindexTaskPOJO(index, request);
//            reindexTask.setReindexParams("From: " + GeneralUtils.convertLongToDateString(fromDate, dateFormat) +
//                    " To: " + GeneralUtils.convertLongToDateString((fromDate + timeChunk), dateFormat));
//            reindexTask.addReprocessParam("from_date", fromDate);
//            reindexTask.addReprocessParam("to_date", toDate);
//            reindexTask.addReprocessParam("date_field", dateField);
//            reindexTask.addReprocessParam("date_format", dateFormat);
//            reindexTasks.add(reindexTask);
//            fromDate += timeChunk;
//        }
//        while (fromDate < toDate);
//        return reindexTasks;
//    }
    private ReindexRequest buildRequest(final String dateField, final String startDate, final String endDate) {
    ...
    private ReindexRequest buildRemoteRequest(final EsSettings source, final String dateField, final String startDate, final String endDate) {
        ReindexRequest request = new ReindexRequest();
