...
import com.electusplus.reindex.reindex_execution_plan_builder.dao.index.CreateIndexDAO;
import com.electusplus.reindex.reindex_execution_plan_builder.reindex_algorithms.IReindexAlgorithm;
...
public class ReindexPlanBuilder {
    ...
    public ReindexPlanBuilder(ProjectPOJO project) {
    ...
    public ReindexPlanPOJO generatePlanFromReprocessing(ProjectStatusPOJO projectStatus) {
        reindexPlan.setReindexJobs(generateReindexJobs(projectStatus));
        return reindexPlan;
    }

    //TODO need to get parameters from monitoring (status etc)
    private Map<String, ReindexJobPOJO> generateReindexJobs(ProjectStatusPOJO projectStatus) {
    ...
    private List<ClusterTaskPOJO> generateClusterTasks() {
        ...
        if (project.getReindexSettings().isTransferIndexSettingsFromSourceIndex()) {
//            if (project.getReindexSettings().isMergeToOneIndex()) {
////                reindexPlan.getReindexJobs()
...
//                        .forEach(job -> {
//                            CloneIndexMetadataDAO cloneIndexMetadata = new CloneIndexMetadataDAO(
//                                    "default",
//                                    job.getIndexName(),
//                                    job.getReindexTasks().get(0).getReindexRequest().getDestination().index());
//                            clusterTasks.add(new ClusterTaskPOJO(cloneIndexMetadata));
//                        });
//            }
        }
        return clusterTasks;
    ...
    private ReindexRequest generateDestination(ReindexRequest request, final String sourceIndex) {
